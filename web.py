...
# Configuration
styles_css = os.path.join("styles.css")
...
def handle_image_click(img, evt: gr.SelectData):
    ...
    for drawn_mask_id in drawn_masks_per_id.keys():
        ...
        if mask[points_coords[1], points_coords[0]]:
            ...
            colors_arr = [mask["last_color"] for mask in drawn_masks_per_id.values()]
    ...
    masked_image = masks_fn.get_masks_img(img, reshaped_masks, color=new_mask_color)
...
with gr.Blocks(css=styles_css) as demo:
            ...
            with gr.Column():
                ...
                with gr.Row():
                    segmented_boxs_image = gr.Image(
                        label="Boxes generated", elem_classes="gr-img-segm-result-boxes"
                    )
    ...
    def start_segmentation(input):
        ...
        if len(input_boxes) == 0:
            ...
            segmented_masks = segment_boxes(input_image, input_boxes, multimask_output)
    ...
    def start_inpainting(original_img, prompt_text, prompt_negative):
        ...
        inpaint_res = inpaint(prompt_text, prompt_negative, b64_img, b64_mask, config)
        ...
        if UIConfig.get_cfg(Setting.UpscaleOutput):
            ...
            if UIConfig.get_cfg(Setting.SaveOutput):
                upscaled_result_write = cv2.cvtColor(upscaled_result, cv2.COLOR_BGR2RGB)
    ...
    send_out_to_in_button.click(
        handle_send_to_input, inputs=result_image, outputs=[image_clicker, image_masker]
    )
